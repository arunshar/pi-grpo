...
\begin{enumerate}[leftmargin=*]
  ...
  \item A \textbf{rollout and checkpoint design} built around vLLM \citep{kwon2023vllm} (measured at $30\times$ the Hugging Face \texttt{generate} fallback at batch $128$ on the 7B policy, Section~\ref{sec:gpu-systems}; the prefix-cache amortization on long shared prompts is a further, separable design target) with a Transformers fallback for tests, and content-addressed checkpoints (\texttt{runs/<id>/step\_<n>/<sha>.bin}) with an append-only audit manifest.
...
\textbf{Setup.} The target full-scale configuration is \texttt{Qwen2-7B-Instruct} \citep{qwen2024qwen2} as both policy and frozen reference, fine-tuned on 11k preference triples from a 30-day GeoTrace-Agent HITL export ($\sim$8k natural HITL labels and $\sim$3k synthesized via the curator's $K\!=\!8$ rollout-and-rank), on 1$\times$H100 80GB for training with 1$\times$H100 hosting vLLM with prefix caching. The GRPO arm of this configuration has now been built and run: a $1{,}200$-step \texttt{Qwen2-7B-Instruct} + LoRA ($r{=}16$, $\alpha{=}32$) policy was trained under the physics reward on 1$\times$H100 (Hugging Face \texttt{generate} rollouts, $B{=}8$, $K{=}4$, horizon $24$, $\sim$51\,min), and its measured learning curve is reported in Section~\ref{sec:llmrun}. The PPO and DPO trainer arms and the cross-method comparison of Table~\ref{tab:expected-trends} share the same entrypoint but have not yet been run, so those trained-policy violation rates remain diagnostic targets; the staging plan is documented in \texttt{docs/EXP5\_LLM\_SCALE.md}. The remaining results in this section are CPU-reproducible evidence that the method and infrastructure are correct: a property and unit suite, a measured reward-level reward-hacking probe, a measured reward-actor scaling crossover (Section~\ref{sec:scaling}), hot-path microbenchmarks, and the two-item golden evaluator.

\smallskip
\textbf{Measured vs.\ target.} Every number in this section is labeled either \emph{measured} (produced by a repository script, reproducible on CPU) or a \emph{diagnostic target} (the direction a correct full-scale run should follow, not yet observed). The reward-actor scaling and its $B\!\cdot\!K$ and heavy-reward crossovers (Tables~\ref{tab:scaling}, \ref{tab:crossover}, and~\ref{tab:heavy}), the asynchronous staleness sweep (Table~\ref{tab:r2}), the reward-hacking probe (Table~\ref{tab:hack}), the repository checks (Table~\ref{tab:repo-checks}), and the microbenchmarks are measured, as is the full-scale 7B GRPO learning curve of Section~\ref{sec:llmrun}; the cross-method trend table (Table~\ref{tab:expected-trends}) and the ablation matrix (Table~\ref{tab:ablation-matrix}) remain diagnostic targets pending the PPO and DPO trainer arms.
...
The queue behaves as designed: mean lag climbs from $0$ (synchronous) to $0.5$ at $s{=}1$ and saturates near $1.47$ for $s\ge2$, while the on-policy fraction collapses from $1.00$ to $0.50$ to $\sim$$0.017$. By $s{=}2$, under $2\%$ of gradient steps consume a sample generated by the current policy version, so training is almost entirely off-policy. Despite that, the final reward is invariant: the reward gap versus the synchronous baseline is $0.000$ at every $s$ (all runs converge to the same level, reward rising from $-311.5$ to $\approx 0$), and the mean KL does not inflate with staleness; it is flat-to-slightly-lower ($1.970 \to 1.860$), consistent with staler, more-averaged updates being marginally smoother here rather than more divergent. The knee $s^{*}$, defined as the largest staleness whose reward stays within tolerance of $s{=}0$, is therefore the top of the sweep, $s^{*}{=}8$. This is a methodological result rather than a tuned operating point: the in-process task is easy enough that no swept staleness degrades reward, so $s^{*}$ confirms the harness can locate the knee while signaling that a harder task (the 7B path) would move it inward.
...
\subsection{Full-scale 7B GRPO run (measured)}
\label{sec:llmrun}
We trained the GRPO arm end to end on a real LLM policy: \texttt{Qwen2-7B-Instruct} with a LoRA adapter ($r{=}16$, $\alpha{=}32$), Hugging Face \texttt{generate} rollouts, $B{=}8$ prompts and $K{=}4$ rollouts per step, horizon $24$ motion-primitive ids, $1{,}200$ steps on a single H100 in $\sim$51\,min (\texttt{scripts/grpo\_llm\_train.py}; the adapter and a per-step \texttt{metrics.jsonl} are saved under \texttt{exp5\_full/}). The reward improves substantially out of the exploration phase: mean group reward rises from $-483.0$ at step $0$ to $-94.5$ at step $1{,}199$ (best $-93.3$ at step $1{,}151$), a $+388$ gain, with most of the climb between steps $60$ and $150$ ($-300$ first crossed at step $61$, $-150$ at step $121$, $-100$ at step $154$) followed by a long plateau. The run completes without the frozen-gradient collapse that destroyed an earlier unstabilized attempt, confirming that the KL-stability machinery and a hotter sampler ($T{=}1.1$, top-$p{=}0.95$) train a real 7B policy to a large reward gain rather than diverging.

Two findings are reported honestly rather than smoothed. First, the binary hard-violation rate stays at $1.0$ for the entire run: at horizon $24$ essentially every sampled trajectory still trips at least one hard-constraint primitive, so the policy never reaches full feasibility. What it learns instead is to shrink the \emph{magnitude} of infeasibility, which is precisely what the $+388$ reward gain measures (the unbounded hard penalty falling from $\sim$$-480$ toward $\sim$$-94$); at this horizon the binary rate is the wrong success axis and the penalty magnitude is the right one, so we report the latter. Second, the within-group reward standard deviation collapses from $\sim$$124$ to $\sim$$2.2$ as the $K$ rollouts converge, and this is what ends the learning phase: once the group agrees, the GRPO advantage shrinks toward zero and the policy plateaus. The hotter sampler delays this collapse just long enough (past step $\sim$$150$) to bank the reward gain, but does not prevent it; recovering further gains would require sustained group diversity (higher $K$, entropy bonus, or curriculum) rather than more steps.

\smallskip
\textbf{KL drift (measured on the 7B run).} The bounded \texttt{AdaptiveKLController} prevents the runaway divergence that destroys an unbounded run (in unbounded PPO ablations KL spikes above $50$ within $500$ steps and the value-head loss diverges, the canonical failure mode), but on the 7B GRPO run it does \emph{not} hold KL inside the $[3,8]$ target band: the KL coefficient saturates at its clamp ceiling ($2.0$) by step $\sim$$140$ and the sequence-level KL to the frozen reference settles near $19$ (mean $19.4$ over the last $200$ steps, max $20.9$). Tail stability is therefore enforced by the trust-region brake (skip the update when KL$>16$), which fires on $1{,}057$ of $1{,}200$ steps and on essentially every step once the policy has converged, rather than by the KL penalty pulling KL back to target. The honest reading is that the LoRA policy adapts aggressively away from the reference and the brake, not the penalty, is the binding stability mechanism at this scale; holding the target band would require a higher coefficient ceiling or a smaller learning rate. GRPO's group baseline reaches this bounded-but-saturated regime without a value head.
...
\smallskip
\textbf{Reward-hacking mechanism (reward-level, measured).} The end-to-end trained-policy probe requires a DPO-optimized policy and is part of the pending DPO arm; here we isolate and measure, directly on the reward, the mechanism that the trained probe relies on. We build physically infeasible trajectories ($3\times$ the S-KBM speed cap of $12.9$\,m/s) and attach a large hacked preference logit ($+10$) of the kind a compromised preference model would assign. Under a preference-only reward ($w_{\text{hard}}=0$) every infeasible trajectory is accepted (mean total $+10.0$, $0\%$ caught); under the physics-grounded reward ($w_{\text{hard}}=5$, the repository default) the unbounded hard term (mean $-100$) overrides the hacked signal (mean total $-490.5$), rejecting $100\%$ of the infeasible set while feasible trajectories stay at $+10.0$. Table~\ref{tab:hack} summarizes; \texttt{scripts/verify\_reward\_hacking\_mechanism.py} reproduces it on CPU. The trained-policy violation rate under the $\gamma_{\text{phys}}$ DPO ablation is reported as a diagnostic target in Table~\ref{tab:expected-trends} and will be replaced by a measured value after the DPO run.
...
\smallskip
\textbf{Microbenchmarks (measured, CPU).} The hot-path reward operations are cheap and are not the training bottleneck: the per-step S-KBM evaluator runs at $\sim$23k ops/s ($43\,\mu$s), the full hybrid \texttt{PhysicsReward.score} at $\sim$19k ops/s ($52\,\mu$s), and preference synthesis ($K\!=\!4$) at $\sim$712k ops/s ($1.4\,\mu$s), single-threaded via \texttt{scripts/bench.py}. Rollout throughput, not reward computation, is the online bottleneck, which is why vLLM prefix caching sits on the rollout path rather than the reward path; Section~\ref{sec:gpu-systems} measures that bottleneck directly on the 7B policy.

\subsection{GPU systems measurements: rollout serving, training memory, and kernels (measured)}
\label{sec:gpu-systems}
The CPU evidence above establishes correctness; this subsection reports the GPU-side systems numbers that decide whether the full-scale design is practical, all measured on the real \texttt{Qwen2-7B-Instruct} policy (\texttt{scripts/bench\_vllm.py}, \texttt{scripts/fsdp\_train.py}, \texttt{scripts/profile\_step.py}, \texttt{scripts/triton\_kernel\_demo.py}; Table~\ref{tab:gpu-systems}).

\smallskip
\textbf{Rollout serving is the bottleneck, and vLLM addresses it (measured, H100).} A profiler pass over one GRPO training step ($8$ sequences, $160$ generated motion-primitive ids, a LoRA adapter over the frozen $7.6$B base) achieves $46.2$\,TFLOP/s, a model-FLOP utilization of $4.7\%$ against the $989$\,TFLOP/s bf16 dense peak, with the step dominated by \texttt{aten::mm}. The low MFU is structural, not a defect: a GRPO step spends most of its wall-clock in autoregressive rollout generation rather than in dense forward/backward matmuls, so the optimization lever is rollout throughput, not GEMM efficiency. This is exactly where vLLM \citep{kwon2023vllm} pays off. Serving the same 7B policy, the vLLM engine reaches $13{,}554$ tok/s at batch $128$ versus $451$ tok/s for the Hugging Face \texttt{generate} fallback at batch $8$, a $30\times$ speedup, along a clean batch-size curve ($131 \to 1{,}003 \to 3{,}944 \to 13{,}554$ tok/s at batch $1/8/32/128$). This is the end-to-end rollout-engine speedup measured directly; the prefix-cache amortization of a long shared task prompt is a separable saving on top of it and remains a design target, since this benchmark used distinct prompts.

\smallskip
\textbf{Training fits under full sharding with headroom (measured, A100).} The GRPO trainer runs the 7B policy under PyTorch FSDP \texttt{FULL\_SHARD} with bf16 mixed precision and gradient checkpointing. On a single $4\times$A100 node it sustains $9{,}041$ tok/s global ($2{,}260$ tok/s per GPU) at sequence length $1{,}024$, with a peak of $23.1$\,GB per rank, well inside the $80$\,GB budget, so batch or sequence length can grow before memory binds. The two-GPU NCCL path was validated separately as a correctness smoke of the FSDP wrap (a toy model, not the 7B), and a cross-node ($2$-node, $8$-GPU) strong-scaling point is queued; we therefore report the single-node figure as a measured operating point and make no multi-node scaling-efficiency claim here.

\smallskip
\textbf{Kernel-level headroom (measured, H100).} As an illustration of the on-device optimization surface, a fused Triton RMSNorm kernel matches the eager reference to $2.9\times10^{-6}$ max absolute error while running $3.1\times$ faster ($0.072$ vs.\ $0.223$\,ms at shape $4096\times3584$, fp32). This is one representative kernel rather than an end-to-end training speedup, but it bounds the per-op saving available to a fused-kernel pass.

\begin{table}[t]
\centering
\caption{GPU systems measurements on the real \texttt{Qwen2-7B-Instruct} policy (measured). Serving and profiling on one H100; FSDP training on one $4\times$A100 node. Throughput, memory, step time, and MFU are produced by the named repository scripts. A cross-node FSDP point is queued and intentionally omitted rather than estimated.}
\label{tab:gpu-systems}
\begin{tabular}{lll}
\toprule
Measurement & Result & Script \\
\midrule
vLLM serving (batch $128$, H100)        & $13{,}554$ tok/s                  & \texttt{bench\_vllm.py} \\
HF \texttt{generate} (batch $8$, H100)  & $451$ tok/s                       & \texttt{bench\_vllm.py} \\
vLLM speedup over HF fallback           & $30\times$                        & \texttt{bench\_vllm.py} \\
FSDP training, $4\times$A100, 7B        & $9{,}041$ tok/s ($2{,}260$/GPU)   & \texttt{fsdp\_train.py} \\
FSDP peak memory per rank               & $23.1$\,GB (of $80$)              & \texttt{fsdp\_train.py} \\
GRPO step MFU (H100)                    & $4.7\%$ ($46.2$ TFLOP/s)          & \texttt{profile\_step.py} \\
Fused Triton RMSNorm                    & $3.1\times$ vs.\ eager            & \texttt{triton\_kernel\_demo.py} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Expected trend table for the first full run. Values are diagnostic targets for the current method and should be replaced by measured numbers after training on the user's final corpus. The GRPO arm has since been run at 7B scale (Section~\ref{sec:llmrun}); its measured behavior (a large reward-magnitude gain, a binary hard-violation rate that stays at $1.0$ at horizon $24$, and a saturated KL) refines these targets, while the base, DPO, and PPO rows await their runs.}
